...
#'
#' If `v` is a factor, the returned vector is also a factor (and the levels
#' of the returned value are determined by the `to.vec` argument), otherwise
#' class of the returned vector is determined by the `to.vec` argument. If
...
#' will be mapped to in `to.vec`. Alternatively, a character vector of the
#' same length as `to.vec` of only 1-to-1 mapping is desired.
