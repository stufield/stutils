...
#' `SIMPLIFY = FALSE` argument is hard coded but can be set by the user.
#'
...
#' passed sequentially to the `FUN` argument, which is typically the final
#' argument (but does not have to be).
#' @return If `SIMPLIFY = FALSE`, typically a list object with each entry
#' the result of applying the function argument to each element of the
#' `...`. If `SIMPLIFY = TRUE`, an attempt is made to reduce the
#' result to a vector or matrix in a similar fashion to the `simplify=`
...
Mapply <- function(...) {
  ...
  if ( any(grep("SIMP", names(args), ignore.case = TRUE)) ) {
    simpl_idx <- grep("SIMP", names(args), ignore.case = TRUE)
  } else {
    args$SIMPLIFY <- FALSE
    simpl_idx     <- length(args)
  }
  ...
  names(args)[ c(fun.idx, simpl_idx) ] <- c("FUN", "SIMPLIFY")
  Lvec     <- sapply(args[-c(fun.idx, simpl_idx)], length)
  ...
  args <- args[ c(fun.idx, simpl_idx,
                  setdiff(seq_along(args),
                          c(fun.idx, simpl_idx))) ]    # reorder FUN first
