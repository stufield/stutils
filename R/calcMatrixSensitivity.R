...
#'   in the population}
#' @author Stu Field
#' @seealso [analyzeProjectionMatrix()], [eigen()],
#' [popbio::eigen.analysis()]
...
calcMatrixSensitivity <- function(A) {
  w      <- Mod(eigen(A)$vectors[, 1L])    # right eigenvector
  v      <- Mod(eigen(t(A))$vectors[, 1L]) # left eigenvector
  lambda <- Mod(eigen(A, only.values = TRUE)$values[1L])  # dominant eigenvalue
