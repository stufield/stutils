...
plotErrorBars <- function(x, lo, up, plotfun = graphics::barplot,
  ...
  for ( i in seq_along(y) ) {
    lines(c(y[i], y[i]), c(up[i], lo[i]), col = bar.col,
          lty = bar.lty, lwd = bar.lwd)      # error bar
    lines(c(y[i] - g, y[i] + g), c(up[i], up[i]),
          col = bar.col, lty = bar.lty, lwd = bar.lwd)  # top bar
    lines(c(y[i] - g, y[i] + g), c(lo[i], lo[i]),
