...
#' @param plot Plot both histogram & drift with time (default) or just
#' histogram ("h") or just line graph ("l")
#' @return %% ~Describe the value returned
#' @note sgf@colostate.edu
#' @author Stu Field
...
DriftSim <- function(p.star= 0.5, n= 50, nsim=50, plot= "b") {
   ...
   ########################################
   ### Plotting drift of the p-allele
   ### in each simulation
   #####################################
   if ( plot == "l" | plot == "b" ) {
      plot(1:clip, pMat[,1],
      ...
      for (n in 1:nsim) {  lines(1:clip, pMat[,n], col= n)   } 
   }
}
