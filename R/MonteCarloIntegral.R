...
MonteCarloIntegral <- function(n=10000, interval, FUN,
   ...
   if ( plot && quick ) {
      ...
      abline(v=interval, lty=2, lwd=1.5); box()
      points(x[which(y<f.x)], y[which(y<f.x)], cex=0.2, pch=20, col=cols[1])
      points(x[which(y > f.x)], y[which(y > f.x)], cex=0.2, pch=20, col=cols[2]) 
      title(main=sprintf("Integral (Area) = %0.4f", tot_area*p_hits),
            font.lab=4, cex.lab=1.5)   
   }
